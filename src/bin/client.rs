...
async fn main() {
    ...
    loop {
        ...
        if opcode == "set".to_string() {
            ...
            file.write_all(req.clone().op.as_bytes()).expect("write failed");
            file.write_all(" ".as_bytes()).expect("write failed");
            file.write_all(req.clone().key.as_bytes()).expect("write failed");
            file.write_all(" ".as_bytes()).expect("write failed");
            file.write_all(req.clone().val.as_bytes()).expect("write failed");
